...
from app.repositories.postgresql import content_repository
...
async def resize_weight(action_log_repo: ActionLogRepository, user_weight_repo: UserWeightRepository):
  ...
  for user_id, logs in grouped_logs.items():
    ...
    for log in logs:
      ...
      value = int(log['value'])
      weight = weight_strategy.convert_to_weight(type, value)
      weight = exponential_decay_weight(weight, log['timestamp'])
      content_id = log['contentId']
      genres = await content_repository.get_genres_by_content_id(content_id)
      for genre in genres:
